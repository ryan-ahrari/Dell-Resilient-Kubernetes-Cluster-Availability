...
def get_all_etcd_pods():
    ...
    # v1 = client.CoreV1Api()
    running_pods = v1.list_pod_for_all_namespaces(watch=False)
...
def is_unhealthy1(mdict):
    #print('One')
    if mdict.get('etcd_server_has_leader') < 1:
        #print('Unhealthy:', 'etcd_server_has_leader')
        return True
    if mdict.get('etcd_server_leader_changes_seen_total') >= 500:
        #print('Unhealthy:', 'etcd_server_has_leader')
        return True

    if (mdict.get('etcd_server_proposals_committed_total') - mdict.get('etcd_server_proposals_applied_total')) > 2000:
        #print('Unhealthy:', 'proposals_committed_total >> proposals_applied_total')
        return True

    if mdict.get('process_open_fds') > mdict.get('process_max_fds') * 0.1:
        #print('Unhealthy:', 'process_open_fds getting too large')
        return True

    if (mdict.get('etcd_disk_backend_commit_duration_seconds_sum') / mdict.get('etcd_disk_backend_commit_duration_seconds_count')) > 0.05:
        #print('Unhealthy:', 'etcd_disk_backend_commit_duration average too slow')
        # print('etcd_disk_backend_commit_duration Average:', mdict.get('etcd_disk_backend_commit_duration_seconds_sum') / mdict.get('etcd_disk_backend_commit_duration_seconds_count'))
        return True

    if (mdict.get('etcd_disk_wal_fsync_duration_seconds_sum') / mdict.get('etcd_disk_wal_fsync_duration_seconds_count')) > 0.05:
        #print('Unhealthy:', 'etcd_disk_wal_fsync_duration average too slow')
        # print('etcd_disk_wall_fsync_duration Average:', mdict.get('etcd_disk_wal_fsync_duration_seconds_sum') / mdict.get('etcd_disk_wal_fsync_duration_seconds_count'))
        return True

    #print('\nDone Checking Threshholds...')
    return False

# Threshold helper function 2
def is_unhealthy2(mdict, old_metrics):
    #print('Two')
    if mdict.get('etcd_server_has_leader') < 1:
        #print('Unhealthy:', 'etcd_server_has_leader')
        return True

    if mdict.get('etcd_server_leader_changes_seen_total') - old_metrics.get('etcd_server_leader_changes_seen_total') > 3:
        #print('Unhealthy:', 'etcd_server_has_leader')
        return True
...
# Function to get unhealthy member using RAFT metrics, pass in etcd cluster endpoint
def get_metrics(etcd_endpoint):
    keys = []
    metrics_list = ['has_leader', 'leader_changes_seen_total', 'proposals_committed_total', 'proposals_applied_total', 'proposals_pending', 'proposals_failed_total', 'wal_fsync_duration_seconds', 'backend_commit_duration_seconds', 'peer_round_trip_time_seconds','peer_sent_failures_total', 'peer_received_failures_total','snapshot_save_total_duration_seconds', 'process_open_fds', 'process_max_fds']

    etcd_cluster_endpoint = 'http://'+etcd_endpoint+':2379'
    etcd_pods = get_all_etcd_pods()
    etcd_cluster_members = requests.post(etcd_cluster_endpoint+"/v3/cluster/member/list").json()["members"]
    running_etcd_members = list(member for member in etcd_cluster_members if ("name" in member) and ("clientURLs" in member))
    running_etcd_members_names = list(member["name"] for member in etcd_cluster_members if ("name" in member) and ("clientURLs" in member))  # change this to "in running_etcd_members"
    etcd_members_not_brought_up = list(member for member in etcd_cluster_members if ("name" not in member) or ("clientURLs" not in member))  # set difference
    functioning_etcd_pods = [etcd_pod for etcd_pod in etcd_pods if etcd_pod.spec.hostname in running_etcd_members_names]
    ...
    for etcd_pod in functioning_etcd_pods:
        ...
        if etcd_pod.status.pod_ip:
            res = requests.get("http://"+etcd_pod.status.pod_ip+":2379/metrics", timeout=None)
            res=res.text

            # print('POD METRICS', pod_num, '=====================================================================================')

            all_metrics = res.split('\n')
            mdict  = {} # Dictionary holding the Raft etcd metrics we want to look at
            for metric in all_metrics:
                if check_space(metric) == 1:
                    for wanted_metric in metrics_list:
                        if wanted_metric in metric:
                            pair = metric.split()
                            mdict[pair[0]] = float(pair[1])
                            # print(pair[0], '->', pair[1])
                            if pair[0] not in keys:
                                keys.append(pair[0])


            # print('Pod metrics:', etcd_pod.metadata.name)

            unhealthy = False
            old_metrics = pod_metrics.get(etcd_pod.metadata.name, None)

            if old_metrics is None:
                unhealthy = is_unhealthy1(mdict)
            else:
                unhealthy = is_unhealthy2(mdict, old_metrics)

            pod_metrics[etcd_pod.metadata.name] = mdict

            if unhealthy:
                failed_etcd_pod_names.append(etcd_pod.metadata.name)

            # pod_num += 1

    # print('DONE')
    return failed_etcd_pod_names

if __name__ == '__main__':
	# Will point to the kubernetes cluster from ~/.kube/config (kubectl config view)
	config.load_kube_config()
	v1 = client.CoreV1Api()
	appsV1 = client.AppsV1Api()
	custApi = client.CustomObjectsApi()

	SCHEDULABLE_CONTROL_PLANE = False
	FTT = 1

	# In our case, this is the etcd-client service
	etcd_cluster = get_etcd_client_service()
	etcd_cluster_ip = etcd_cluster.spec.cluster_ip
	etcd_cluster_port = etcd_cluster.spec.ports[0].port
	etcd_cluster_endpoint = f'http://{etcd_cluster_ip}:{etcd_cluster_port}'

	EtcdApiHandler = EtcdApiHandler(url=etcd_cluster_endpoint)
	...
	ETCD_DATA_DIRECTORY = '/root/data/myss'
	ETCD_INITIAL_CLUSTER_TOKEN = 'etcd-cluster-1604'

	read_get_lock = threading.Lock()

	pending_migrations_queue = EventQueue()

	# b = threading.Thread(name='back', target=migrate_pod, args=("etcd0-0",))
	# # b.start()
	# update_cluster()

	# node_event_monitoring_thread = threading.Thread(name='node_monitoring', target=watch_nodes)
	etcd_monitoring_thread = threading.Thread(name='etcd_monitoring', target=etcd_health_monitoring)
	event_queue_monitoring_thread = threading.Thread(name='event_queue_monitoring', target=event_queue_monitoring)
	get_metrcs_thread = threading.Thread(name='get_metrics', target=get_metrics, args=(str(etcd_cluster_ip)))
	# node_event_monitoring_thread.start()
	etcd_monitoring_thread.start()
	event_queue_monitoring_thread.start()
	get_metrcs_thread.start()
	# update_cluster()
